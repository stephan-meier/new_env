...
POSTGRES_CONN = "host=postgres dbname=demo user=demo_user password=demo_pass"

# Postgres-adaptierte DDL (aus Snowflake konvertiert)
CREATE_TABLES_SQL = """
...
with DAG(
    ...
    create_tables = BashOperator(
        ...
        bash_command=f'psql "{POSTGRES_CONN}" -c "{CREATE_TABLES_SQL}"',
    )
    ...
    for table_name, csv_file in TABLES:
        copy_task = BashOperator(
            ...
            bash_command=(
                f'psql "{POSTGRES_CONN}" -c "'
                f"COPY raw.{table_name} FROM '/data/{csv_file}' "
                f"WITH (FORMAT csv, HEADER true, DELIMITER ',', QUOTE '\"');"
                f'"'
            ),
        )
